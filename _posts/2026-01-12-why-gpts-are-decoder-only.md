...
* It must learn the conditional distribution $P(x_t \mid x_{<t})$, not an easier version that leaks future information during training.
...
The goal of a language model is to learn the conditional distribution $P(x_t \mid x_{<t})$. If the model is allowed to see future tokens during training, it will learn a different and easier distribution $P(x_t \mid x_{<t}, x_{>t})$. This may produce lower training loss, but it breaks the model.
